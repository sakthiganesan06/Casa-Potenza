...
def run(results: list[ExampleResult], workers: int) -> dict:
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as pool:
        ...
        for future in as_completed(futures):
            ...
            except judge.JudgeNotConfigured as e:
                errors += 1
                last_error = str(e)

    if errors:
        msg = f"Judge error ({last_error}) -- judge-based checks skipped." if last_error else "Judge not configured -- judge-based checks skipped."
        return {
    ...
    if not verdicts:
        return {
            ...
            "num_confident_answers": 0,
            "self_report_precision": 1.0,
