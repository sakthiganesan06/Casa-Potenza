...
def print_report(report: dict):
    ...
    elif f.get("num_evaluated", 0) == 0:
        print("  0 answers evaluated (or judge skipped)")
    else:
        print(f"  {f['num_evaluated']} answers evaluated")
        print(_gap_row("Faithful rate", f.get("faithful_rate", 1.0), 1.0))
        print(_gap_row("Hallucination rate", f.get("hallucination_rate", 0.0), 0.0))
        print(_gap_row("Self-report precision", f.get("self_report_precision", 1.0), 1.0))
        print(
            f"  (self-report precision: of the {f.get('num_confident_answers', 0)} answers the system itself marked "
            f"grounded=True, how many the judge also confirmed faithful -- refusals excluded, see "
            f"eval/checks/faithfulness.py)"
        )
        if f.get("hallucinated_examples"):
    ...
    elif c.get("num_evaluated", 0) == 0:
        print("  0 answers evaluated (or judge skipped)")
    else:
        print(f"  {c['num_evaluated']} answerable-query answers evaluated")
        print(_gap_row("Correct rate", c.get("correct_rate", 0.0), 1.0))
        if c.get("incorrect_examples"):
